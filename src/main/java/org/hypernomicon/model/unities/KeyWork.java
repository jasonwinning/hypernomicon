...
//---------------------------------------------------------------------------
  ...
  public KeyWork(HDT_RecordWithAuthors<? extends RecordAuthors> recordWithAuthors)
  {
...
//---------------------------------------------------------------------------
  ...
  private boolean updateSearchKeyAndCheckIfActive()
  {
    ...
    HDT_RecordWithAuthors<? extends RecordAuthors> record = getRecord();
    ...
    {
      Keyword keyword = db.getKeyByKeyword(searchKey);

      if (keyword != null)
      {
        if (keyword.getAllRecords().contains(record))
    ...
    if (record.getType() == hdtWork)
    {
      ...
      if ((work.getAuthors().isEmpty() == false) && (work.getYearStr().isEmpty() == false))
      {
        ...
        for (KeywordBinding binding : work.getSearchKeys())
        {
          ...
          for (WorkSearchKeyConfig keyConfig : settings)
          {
            ...
            String keyUserText = binding.getUserText();

            // Remove letter from active keyword if one was added

            int letterPos = keyConfig.parentheses == CitationParenthesesOption.none ? keyUserText.length() - 1 : keyUserText.length() - 2;

            if (safeSubstring(keyUserText, letterPos - work.getYearStr().length(), letterPos).equals(work.getYearStr()))
              keyUserText = keyUserText.substring(0, letterPos) + safeSubstring(keyUserText, letterPos + 1, keyUserText.length());
            ...
            {
              if (keyUserText.equals(keyConfig.format(List.of(author.singleName()), work.getYearStr())))
              {
                searchKey = binding.getUserText();
                return true;
              }
            }
          }
