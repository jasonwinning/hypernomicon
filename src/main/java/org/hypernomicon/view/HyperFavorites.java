      ...
      else if ("query".equals(node.node("favTypes").get(String.valueOf(ndx), "")))
      {
        ...
        for (int rowNdx = 0; rowNdx < rowCount; rowNdx++)
        {
          ...
          for (int colNdx = 0; colNdx < QUERY_FAV_ROW_COLUMN_COUNT; colNdx++)
          {
            ...
            row.cells[colNdx] = colNdx < 2 ?
              new GenericNonRecordHTC(id, text, "all".equals(typeStr) ? hdtNone : Tag.parseTypeTagStr(typeStr))
            :
              new RecordHTC(id, text, "all".equals(typeStr) ? hdtNone : Tag.parseTypeTagStr(typeStr));
          }
