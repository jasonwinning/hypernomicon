...
//---------------------------------------------------------------------------
  ...
  /** Sets a cell factory that displays a tooltip matching the cell text. */
  public static <S, T> void setTooltipCellFactory(TableColumn<S, T> column)      { setTooltipCellFactory(column, Object::toString); }

  /** Sets a cell factory that displays a tooltip matching the cell text. */
  public static <S, T> void setTooltipCellFactory(TreeTableColumn<S, T> column)  { setTooltipCellFactory(column, Object::toString); }

//---------------------------------------------------------------------------
//---------------------------------------------------------------------------

  /**
   * Sets a cell factory that displays a tooltip generated by the given function.
   * The cell text is always {@code item.toString()}; the tooltip text comes from {@code tooltipFn}.
   *
   * @param column    the column to configure
   * @param tooltipFn function that converts the cell value to tooltip text (may return null)
   */
  public static <S, T> void setTooltipCellFactory(TableColumn<S, T> column, Function<T, String> tooltipFn)
  {
    column.setCellFactory(col -> new TableCell<>() { @Override protected void updateItem(T item, boolean empty)
    {
      super.updateItem(item, empty);
      applyTooltipToCell(this, item, empty, tooltipFn);
    }});
  }
...
//---------------------------------------------------------------------------
   ...
   * @param tooltipFn function that converts the cell value to tooltip text (may return null)
   */
  public static <S, T> void setTooltipCellFactory(TreeTableColumn<S, T> column, Function<T, String> tooltipFn)
  {
    column.setCellFactory(col -> new TreeTableCell<>() { @Override protected void updateItem(T item, boolean empty)
    {
      super.updateItem(item, empty);
      applyTooltipToCell(this, item, empty, tooltipFn);
    }});
...
//---------------------------------------------------------------------------
  ...
  private static <T> void applyTooltipToCell(Labeled cell, T item, boolean empty, Function<T, String> tooltipFn)
  {
    if (empty || (item == null))
    {
      cell.setText(null);
    ...
    else
    {
      ...
      setToolTip(cell, tooltipFn.apply(item));
    }
  }
