...
var ViewHistory = function () {
  ...
  _createClass(ViewHistory, [{
    ...
    value: function _writeToStorage() {
      ...
      return new Promise(function (resolve) {
        var databaseStr = JSON.stringify(_this2.database); // [Jason Winning+3] ignore errors
        try {
    ...
    value: function _readFromStorage() {
      return new Promise(function (resolve) {
        ...
        if (!value) {
          var databaseStr; // [Jason Winning+3] ignore errors
          try {
            databaseStr = localStorage.getItem('database');
          } catch (e) { }
          if (databaseStr) {
            try {
              var database = JSON.parse(databaseStr);
